$(document).on('pagecreate', "#AsstGJChgList",function (e) {
// 좌우 스윕 이벤트
    $("#AsstGijumChglistview").on("swipeleft", function(e) {
         ...
         if(cur_page == last_page){
             ...
             cur_page++;
             wsAsstGijumChgList(cur_page);
         }
    });

    $("#AsstGijumChglistview").on("swiperight", function(e) {
         if(last_page<2)    return; //한페이지면 필요없음?
...
$(document).on('pagebeforeshow', "#AsstGJChgList",function () {
...
$(document).on('pageshow', "#AsstGJChgList",function (){
    wsAsstGijumChgList(1);
});

$(document).on('pagehide', "#AsstGJChgList",function () {
    $("#AsstGJChgList").remove();
});

var cur_page = 1;//current page
var last_page = 1;//last page

var initAsstChgGJList = function() {
// 화면모드처리
     if(isDone == "B"){
    ...
    $("#btChgDoneSearch").on("click", function(e) {
...
//이전버튼
    $("#btnAsstGJChgList").click(function(){
         SetPcIdx("");
         isDone = "C";
         PageNonChange("AsstChgMain.html");
    });
...
var goAsstChgInfoA = function(pcIdx){
    SetPcIdx(pcIdx);
    isDone = "C"; //자산정보 메뉴
    /*if(GetComType() == "Q"){
    ...
    PageNonChange("AsstChgInfo_P.html");
}

function wsAsstGijumChgList(page) {

gfn_startLoading();

cur_page = page;//현재페이지 동기화
    var url = ws_url + 'GetGijumAsstChgList';
    ...
    var data = "{comCd:'" + GetComCd() + "', gijumCd:'"+GetGijumCd()+"', strSearch:'" + strSearch +"', page: '"+page+"'}";

    var sucess = function(json){

        var list = $("#AsstGijumChglistview");
        ...
        $.each(Table, function(key, val){
           ...
           // }
            ...
            else
            {
                 ...
                 var button = $("<tr class='trow'><td onclick='goAsstChgInfoA({0});' style='width:150px;'>{1}</td><td onclick='goAsstChgInfoA({0});'>{2}</td></tr><tr><td onclick='goAsstChgInfoA({0});' style='width:150px;'>{3}</td><td onclick='goAsstChgInfoA({0});'>{4}</td></tr><tr><td onclick='goAsstChgInfoA({0});' style='width:150px;'>{5}</td><td onclick='goAsstChgInfoA({0});'>{6}</td></tr><tr><td colspan='2' onclick='goAsstChgInfoA({0});'>{7}</td></tr>".format(pcIdx, fn_nvl(pcNum,"&nbsp;" ), fn_nvl(pcSn,"&nbsp;" ), fn_nvl(pcTyp,"&nbsp;" ), fn_nvl(pcModel,"&nbsp;" ), fn_nvl(partNm,"&nbsp;" ), fn_nvl(userNm,"&nbsp;" ), fn_nvl(statNm,"&nbsp;" )));
...
//SN = "B381156";
        ...
        if(!fn_isNull(SN))
        {
           wsChkChgAsstSearCh(SN, function(val, chkGijumCd, chkPcStatus){
                   //debugger;
                   if(val == 0)
                  {
                      ...
                      SetPcNum("");
                       ...
                       alert("조회된 자산이 없습니다.\r\n자산번호나 SN을 정확히 입력해 주십시요");
                  }
                  else
                  {
                       if(chkPcStatus == "C")
                       {
                            alert("해당 자산은 이미 설치완료된 자산입니다.");
                       }
                       else
                       {
...
/// 자산정보 웹서비스 //////////////////////////////////////////
function wsChkChgAsstSearCh(txtSch, callback) {

var url = ws_url + 'GetChgAsstSearch';
